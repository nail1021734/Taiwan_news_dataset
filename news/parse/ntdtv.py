...
REPORTER_PATTERNS: List[re.Pattern] = [
    ...
    # 2021-05-28-103129916`.
    re.compile(
        r'\(?(?:[這这]是)?新[唐塘]人(?:特約)?[記记]?者?(?:亞太)?(?:[電电][視视][台臺]?)?'
        + r'([\w、\s]*?)的?(?:[综綜]合|整理|[採采][訪访])?[報报][導导道]。?\)?'
    ),
    # This observation is made with `url_pattern = 2021-06-28-103153088`.
    re.compile(r'\(?記者(\S+?)(?:[综綜]合|整理|[採采][訪访])?[報报][導导道]\)?'),
    # This observation is made with `url_pattern = 2021-06-28-103153088`.
    re.compile(r'\(?\/?責任編輯:(\S+?)[\)。]'),
