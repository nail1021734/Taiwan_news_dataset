...
def test_module_function_signature() -> None:
    ...
    assert (
        inspect.signature(news.crawlers.chinatimes.get_news_list)
        ==
        Signature(
            parameters=[
                Parameter(
    ...
    assert (
        inspect.signature(news.crawlers.chinatimes.main)
        ==
        Signature(
            parameters=[
                Parameter(
...
def test_module_attribute_signature() -> None:
    ...
    assert (
        news.crawlers.chinatimes.COMPANY_URL
        ==
        news.crawlers.util.normalize.get_company_url(
            company_id=news.crawlers.chinatimes.COMPANY_ID,
        )
    )
