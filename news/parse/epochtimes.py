...
REPORTER_PATTERNS: List[re.Pattern] = [
    ...
    # 19-12-13-11720728`.
    re.compile(r'[\(【]大[紀纪]元?[\w]*[記记]者([\w、,/\s]*?)(?:美國.*?)?(?:採訪|編譯)?(?:報導)?[\)】]'),
    ...
    re.compile(r'\(英文大紀元資深記者(\w*)\(.*\)採訪報導/[\w]*編譯\)英文《大紀元.*》資深記者\w*\([\s\w]*\)'),
...
ARTICLE_SUB_PATTERNS: List[Tuple[re.Pattern, str]] = [
   ...
   # This observation is made with `url_pattern = 20-1-1-11760821`.
    ...
    # 19-5-10-11248624`.
    (
        re.compile(r'(?:現場來賓:(\w|\W)*|\s*好的,感謝您關注新聞看點(,別忘了轉發點[讚贊])?,再會。\s*|\s我們在'
        +r'本期節目結尾.*)?[#]?((?:大紀元《新聞看點》|新唐人(?:《熱點互動》|'
        +r'《新聞拍案驚奇》|《新聞大破解》|《世事關心》))製作組)*[#]?'),
        '',
    ),
    ...
    # 19-12-23-11739348`. 
    (
        re.compile(r'(觀眾朋友|大家|觀眾朋友大家)好[,!](歡迎(?:(大家)?(訂閱)?關注新聞看點,?|收看(?:舊金山焦點|'
        + r'《新聞拍案驚奇》|這一期的【熱點互動】)),|我們現在進入「新拍互動」環節。)?(我是\w*[。,](?:今天都好嗎\?\s)?|《新聞拍案驚奇》獲授權獨家首發:)?'),
        '',
    ),
    # This observation is made with `url_pattern = 19-12-27-11750457,
    # 19-12-28-11751523, 19-12-18-11730757, 19-10-16-11592527`
    (
        re.compile(r'(?:歡迎訂閱【三國英雄】|請看視頻(?:~|《以史為鑒》。))?\s*新唐人、大紀元《三國英雄》聯合製作組'),
        '',
    ),
    ...
    # 19-4-23-11206720, 19-4-21-11201750, 19-1-4-10954370, 19-12-26-11746439,
    # 19-11-25-11678991, 19-12-24-11743464, 19-12-27-11750145, 19-12-30-11754661
    # , 20-1-1-1176083, 19-12-29-11753517, 19-12-28-11751599, 19-12-18-11729919,
    # 19-9-4-11497783, 19-12-11-11715330, 19-12-7-11707556, 19-12-2-11694728,
    # 19-11-28-11688048, 19-11-24-11677348, 19-11-2-11629034, 19-11-2-11628263,
    # 19-10-11-11581844`.
    (
        re.compile(r'(⊙Ending|網絡收看方式|【直播日期】|《大紀元》讀者還享有|\s*謝謝大家!'
        + r'\s*追查迫害法輪功國際組織\s*|下面是視頻直播。|如果您有相關問題|\s*[_=]{10,}|'
        + r'\(原文轉自美房置業網|[◊\s@]*選自《明慧針道|\(本系列完結\) 點閱|註:全文鏈接|'
        + r'事據《.*》(後集)?卷?\w?、?|[\w]*小檔案|\s*\(?[此本]文(?:(此文)?發表|刊載)於[\w]*(?:新聞|教育|B4|論壇)版\)?'
        + r'|(\(節錄完\))?\s*[—─]\s*[—─](?:節(?:錄|選)|摘)自|(陳彥玲博士)?【爸媽必修課】|參考資料|【作者簡介】|要瞭解更多有關《.*》|▼).*'),
        '',
    ),
    # This observation is made with `url_pattern = 20-1-1-11759265
    # 19-12-20-11734996, 19-4-21-11201856, 19-4-5-11166312`.
    (
        re.compile(r'(?:大紀元每天為讀者梳理翻牆必看的文章|編(?:輯|者)按|追查國際調查'
        + r'錄音如下|各位同修,各位同胞|以下是播出時間表):'),
        '',
    ),
    ...
    # 19-12-5-11702000`.
    (
        re.compile(r'\s註[:\s\[\d\]]*(?:李洪志師父著作:《轉法輪》|這高度表示安全、牢不可破。)'),
        '',
    ),
    ...
    # 19-12-20-11734006, 19-12-18-11730384, 19-12-18-11731126, 19-10-16-11592972
    # 19-5-13-11255964, 19-5-11-11250223, 20-1-1-11761463, 19-12-30-11756323,
    # 19-12-27-11749380`.
    (
        re.compile(r'(?:文章來源|大紀元新聞網|主辦單位|作者簡介|醫師小檔案|明慧學校夏'
        + r'令營時間|直播日期|有問題請聯繫|參考書目|【漣漪小語】):.*'),
        '',
    ),
    ...
    # 19-12-20-11734996, 19-12-27-11748709, 19-8-14-11453112, 19-12-1-11692633,
    # 19-11-29-11688080, 19-11-13-11653910, 19-11-13-11653876, 19-10-30-11623041
    # 19-10-28-11617434`.
    (
        re.compile(r'\((?:澳洲墨爾本記者站採訪報導|本文歡迎轉載引用,註明出處'
        + r'即可|簽[名字]|化名|轉改自美國之音|(?:未|本[章卷])完,(全文)?待續|網站專文|特別銘謝:.*|《.*敬請關注.*|敬請繼續關注.*|全文完|接[下上]文|前言)\)'),
        '',
    ),
    # This observation is made with `url_pattern = 19-12-20-11735692,
    # 19-12-15-11723411.`
    (
        re.compile(r'(接上文:)?【(十字路口|美國思想領袖)\s?】[\w\s]*\([上中]\),?'),
        '',
    ),
    # This observation is made with `url_pattern = 19-12-15-11723411'`.
    (
        re.compile(r'以下視頻是採訪的第[二三]部分\(中文字幕\):(?:中共就是個黑幫組織|面對'
        + r'殘暴年輕人毫不退縮)。'),
        '',
    ),
    # This observation is made with `url_pattern = 19-12-14-11722898`.
    (
        re.compile(r',您去大紀元、新唐人網站,就可以看到活動的具體訊息。好的非常感'
        + r'謝各位的收看,我們下次節目再見。\s?'),
        '',
    ),
    ...
    # 19-12-30-11754199, 19-12-13-11721380, 19-12-8-11708626, 19-12-16-11725356,
    # 19-12-11-11714906, 19-11-15-11657230, 19-6-1-11293517`.
    (
        re.compile(r'[-─—\*@\s\(]*點閱[《「【]?[^】」》]*[】」》]?(相關)?(?:(的亮點)?系列(文章)?|連載文章。|徵文)?[-─—\)]*'),
        '',
    ),
